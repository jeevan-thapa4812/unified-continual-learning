...
class DomainBuffer:
    ...
    def __init__(self, max_size, device, input_size, num_classes, num_features, domain_id, examples=None, preds=None,
    ...
    def add_data(self, examples, preds, feats, labels):
        ...
        assert examples.shape[0] == preds.shape[0] == feats.shape[0] == labels.shape[
        ...
        if examples.shape[0] <= self.buffer_size:
            ...
            self.examples[:mem_size], self.preds[:mem_size], self.feats[:mem_size], self.labels[
                                                                                    :mem_size] = examples, preds, feats, labels
        else:
    ...
    def collect_data(self):
        return self.examples[self.indices], self.preds[self.indices], self.feats[self.indices], self.labels[
            self.indices]
